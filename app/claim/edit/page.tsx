...
export default function EditPage() {
  ...
  const { data: schoolData, isLoading } = trpc.schools.byIdWithFacts.useQuery(
    { id: schoolId },
    { enabled: !!schoolId }
  );

  // Pre-populate form with existing facts
  useEffect(() => {
    if (schoolData?.facts) {
      const programs: string[] = [];
      for (const fact of schoolData.facts) {
        if (fact.factKey === FACT_KEYS.CONTACT_EMAIL && typeof fact.factValue === "string") {
          setContactEmail(fact.factValue);
        }
        if (fact.factKey === FACT_KEYS.CONTACT_PHONE && typeof fact.factValue === "string") {
          setContactPhone(fact.factValue);
        }
        if (fact.factKey === FACT_KEYS.PROGRAM_TYPE && typeof fact.factValue === "string") {
          programs.push(fact.factValue);
        }
        if (fact.factKey === FACT_KEYS.FLEET_AIRCRAFT && Array.isArray(fact.factValue)) {
          setFleetAircraft(fact.factValue.join(", "));
        }
        if (fact.factKey === FACT_KEYS.FLEET_COUNT && typeof fact.factValue === "number") {
          setFleetCount(fact.factValue.toString());
        }
        if (fact.factKey === FACT_KEYS.COST_BAND && typeof fact.factValue === "string") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const facts: Array<{ factKey: string; factValue: unknown }> = [];
    ...
    try {
      const response = await fetch("/api/claim/edit", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        setSubmitStatus("error");
        setErrorMessage(data.error || "Failed to submit edits");
      } else {
      ...
      <div className="container mx-auto py-8 px-4 max-w-2xl">
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground">No school ID provided.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-2xl">
      <Card>
        <CardHeader>
          <CardTitle>Edit {schoolData.school.canonicalName}</CardTitle>
          <CardDescription>
            Update your school information. Changes will be submitted for moderation.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {submitStatus === "success" ? (
            <div className="space-y-4">
              <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
                <p className="text-sm text-green-800 dark:text-green-200">
                  Your edits have been submitted for moderation. They will be reviewed and published soon.
                </p>
              </div>
              <Button
                onClick={() => {
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="fleetAircraft">Aircraft Types (comma-separated)</Label>
                  <Input
                    id="fleetAircraft"
